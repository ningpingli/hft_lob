        ...
        for fold_result in model_fold_results:
            ...
            for metric, model_value in {
                **evaluation.overall,
                ...
                baseline_value = reference.overall.get(metric)
                if baseline_value is not None and _finite(model_value, baseline_value):
                    fold_deltas.setdefault(metric, []).append(model_value - baseline_value)
                    fold_wins.setdefault(metric, []).append(model_value > baseline_value)
            baseline_daily = {
            ...
            for daily_record in evaluation.daily:
                ...
                for metric, model_value in daily_record.metrics.items():
                    raw_baseline_value = baseline_metrics.get(metric)
                    ...
                    if _finite(model_value, baseline_value):
                        day_wins.setdefault(metric, []).append(model_value > baseline_value)
            baseline_horizon = {
                    ...
                    for date, model_value in horizon_record.daily_values:
                        ...
                        if isinstance(baseline_day_value, (int, float)) and _finite(
                            ...
                            horizon_day_wins.setdefault(key, []).append(
                                model_value > baseline_day_value
                            )
